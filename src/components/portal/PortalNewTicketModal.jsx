...
const typeOptions = [
  ...
  { value: "other", label: "Otro", icon: MoreHorizontal, color: "text-gray-500" },
];
...
const IconSelect = ({ icon: Icon, iconClassName, label, id, options, value, onChange }) => (
  <motion.div variants={fieldItem}>
    ...
    <div className='relative'>
      <Icon
        aria-hidden='true'
        className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 ${iconClassName || "text-gray-400"}`}
      />
      <select
        id={id}
        value={value}
        onChange={onChange}
        className='w-full pl-10 pr-9 py-2.5 border border-gray-200 rounded-lg outline-none appearance-none transition-all duration-150 hover:border-gray-300 focus:border-brand-500 focus:ring-4 focus:ring-brand-100'
      >
        {options.map((opt) => (
          ...
          >
            <div className='flex items-center gap-3 mb-5'>
              ...
              <div className='flex-1 min-w-0'>
                <h2 className='text-xl font-bold text-gray-900 truncate'>Nuevo ticket</h2>
                <p className='text-sm text-gray-400'>Cuéntanos qué necesitas y te contactaremos</p>
              </div>
              <motion.button
                type='button'
                onClick={onClose}
            ...
            >
              <motion.div variants={fieldItem}>
                <label htmlFor={`${formId}-title`} className='block text-sm font-semibold text-gray-700 mb-1.5'>
                  Título
                </label>
                <div className='relative'>
                  <TypeIcon aria-hidden='true' className='absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-gray-400' />
                  <input
                    id={`${formId}-title`}
                    type='text'
                    required
                    ...
                    className='w-full pl-10 pr-3.5 py-2.5 border border-gray-200 rounded-lg outline-none transition-all duration-150 hover:border-gray-300 focus:border-brand-500 focus:ring-4 focus:ring-brand-100'
                  />
                </div>
              </motion.div>
              ...
              <motion.div variants={fieldItem}>
                <div className='flex items-center justify-between mb-1.5'>
                  <label htmlFor={`${formId}-description`} className='block text-sm font-semibold text-gray-700'>
                    Descripción
                  </label>
                ...
                <div className='relative'>
                  <FileText aria-hidden='true' className='absolute left-3.5 top-3 w-4.5 h-4.5 text-gray-400' />
                  <textarea
