...
// Export API - Funciones para descargar archivos de exportación
export const exportAPI = {
  ...
  // Lista de proyectos
  projectsListPdf: async () => {
    const date = new Date().toISOString().split("T")[0];
    await exportAPI.downloadFile(
      "/exports/projects/pdf",
      `proyectos_${date}.pdf`
    );
  },

  projectsListExcel: async () => {
    const date = new Date().toISOString().split("T")[0];
    await exportAPI.downloadFile(
      "/exports/projects/excel",
      `proyectos_${date}.xlsx`
    );
  },

  projectsListCsv: async () => {
    const date = new Date().toISOString().split("T")[0];
    await exportAPI.downloadFile(
  ...
  projectDetailPdf: async (projectId, projectName = "proyecto") => {
    const date = new Date().toISOString().split("T")[0];
    const safeName = projectName.replace(/[^a-z0-9]/gi, "_").toLowerCase();
    await exportAPI.downloadFile(
      `/exports/projects/${projectId}/pdf`,
      `${safeName}_${date}.pdf`
    );
  },

  projectDetailExcel: async (projectId, projectName = "proyecto") => {
    const date = new Date().toISOString().split("T")[0];
    const safeName = projectName.replace(/[^a-z0-9]/gi, "_").toLowerCase();
    await exportAPI.downloadFile(
      `/exports/projects/${projectId}/excel`,
      `${safeName}_${date}.xlsx`
    );
  },

  // Diagramas Gantt
  projectGanttPdf: async (projectId, projectName = "proyecto") => {
    const date = new Date().toISOString().split("T")[0];
    const safeName = projectName.replace(/[^a-z0-9]/gi, "_").toLowerCase();
    await exportAPI.downloadFile(
      `/exports/projects/${projectId}/gantt/pdf`,
      `gantt_${safeName}_${date}.pdf`
    );
  },

  allProjectsGanttPdf: async () => {
    const date = new Date().toISOString().split("T")[0];
    await exportAPI.downloadFile(
      "/exports/gantt/pdf",
      `gantt_todos_proyectos_${date}.pdf`
    );
  },
};
